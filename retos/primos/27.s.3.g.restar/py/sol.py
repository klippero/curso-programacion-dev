...
class Mixto:
    ...
    def to_fraccion(self):
        result = Fraccion(self.__entero * self.__fraccion.denominador() + self.__fraccion.numerador(), self.__fraccion.denominador() )
        return result

    def __add__(self,otro):
        f1 = self.to_fraccion()
        f2 = otro.to_fraccion()
        fraccionResult = f1 + f2
        result = Mixto(fraccionResult)
        result.simplificar()
        result.simplificarAcanonica()
        return result  
